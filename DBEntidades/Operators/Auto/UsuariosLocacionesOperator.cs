...
namespace DbEntidades.Operators
{
    public partial class UsuariosLocacionesOperator
    {
        ...
        public static UsuariosLocaciones GetOneByParameter(string campo, string valor)
        {
            if (!DbEntidades.Seguridad.Permiso("PermisoUsuariosLocacionesBrowse")) throw new PermisoException();
            string columnas = string.Empty;
        ...
        columnas = columnas.Substring(0, columnas.Length - 2);
            DB db = new DB();
            DataTable dt = db.GetDataSet("select " + columnas + " from UsuariosLocaciones where  " + campo + " = \'" + valor + "\'").Tables[0];
            UsuariosLocaciones UsuariosLocaciones = new UsuariosLocaciones();
            if (dt.Rows.Count > 0)
            {
                foreach (PropertyInfo prop in typeof(UsuariosLocaciones).GetProperties())
        ...
        public static List<UsuariosLocaciones> GetAllByParameter(string campo, string valor)
            {
                if (!DbEntidades.Seguridad.Permiso("PermisoUsuariosLocacionesBrowse")) throw new PermisoException();
                string columnas = string.Empty;
                ...
                if (tipo == "String")
                {
                    queryStr = "select " + columnas + " from UsuariosLocaciones where " + campo + " = \'" + valor.ToString() + "\'";
                }
                else
                {
                    queryStr = "select " + columnas + " from UsuariosLocaciones where " + campo + " = " + valor.ToString();
                ...
                foreach (DataRow dr in dt.AsEnumerable())
                {
                    ...
                    UsuariosLocaciones entidad = new UsuariosLocaciones();
                    foreach (PropertyInfo prop in typeof(UsuariosLocaciones).GetProperties())
                    {
                        object value = dr[prop.Name];
                        if (value == DBNull.Value) value = null;
