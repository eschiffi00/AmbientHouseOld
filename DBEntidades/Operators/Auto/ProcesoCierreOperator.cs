...
namespace DbEntidades.Operators
{
    public partial class ProcesoCierreOperator
    {
        ...
        public static ProcesoCierre GetOneByParameter(string campo, string valor)
        {
            if (!DbEntidades.Seguridad.Permiso("PermisoProcesoCierreBrowse")) throw new PermisoException();
            string columnas = string.Empty;
        ...
        columnas = columnas.Substring(0, columnas.Length - 2);
            DB db = new DB();
            DataTable dt = db.GetDataSet("select " + columnas + " from ProcesoCierre where  " + campo + " = \'" + valor + "\'").Tables[0];
            ProcesoCierre ProcesoCierre = new ProcesoCierre();
            if (dt.Rows.Count > 0)
            {
                foreach (PropertyInfo prop in typeof(ProcesoCierre).GetProperties())
        ...
        public static List<ProcesoCierre> GetAllByParameter(string campo, string valor)
            {
                if (!DbEntidades.Seguridad.Permiso("PermisoProcesoCierreBrowse")) throw new PermisoException();
                string columnas = string.Empty;
                ...
                if (tipo == "String")
                {
                    queryStr = "select " + columnas + " from ProcesoCierre where " + campo + " = \'" + valor.ToString() + "\'";
                }
                else
                {
                    queryStr = "select " + columnas + " from ProcesoCierre where " + campo + " = " + valor.ToString();
                ...
                foreach (DataRow dr in dt.AsEnumerable())
                {
                    ...
                    ProcesoCierre entidad = new ProcesoCierre();
                    foreach (PropertyInfo prop in typeof(ProcesoCierre).GetProperties())
                    {
                        object value = dr[prop.Name];
                        if (value == DBNull.Value) value = null;
