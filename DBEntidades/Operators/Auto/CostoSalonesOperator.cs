...
namespace DbEntidades.Operators
{
    public partial class CostoSalonesOperator
    {
        ...
        public static CostoSalones GetOneByParameter(string campo, string valor)
        {
            if (!DbEntidades.Seguridad.Permiso("PermisoCostoSalonesBrowse")) throw new PermisoException();
            string columnas = string.Empty;
        ...
        columnas = columnas.Substring(0, columnas.Length - 2);
            DB db = new DB();
            DataTable dt = db.GetDataSet("select " + columnas + " from CostoSalones where  " + campo + " = \'" + valor + "\'").Tables[0];
            CostoSalones CostoSalones = new CostoSalones();
            if (dt.Rows.Count > 0)
            {
                foreach (PropertyInfo prop in typeof(CostoSalones).GetProperties())
        ...
        public static List<CostoSalones> GetAllByParameter(string campo, string valor)
            {
                if (!DbEntidades.Seguridad.Permiso("PermisoCostoSalonesBrowse")) throw new PermisoException();
                string columnas = string.Empty;
                ...
                if (tipo == "String")
                {
                    queryStr = "select " + columnas + " from CostoSalones where " + campo + " = \'" + valor.ToString() + "\'";
                }
                else
                {
                    queryStr = "select " + columnas + " from CostoSalones where " + campo + " = " + valor.ToString();
                ...
                foreach (DataRow dr in dt.AsEnumerable())
                {
                    ...
                    CostoSalones entidad = new CostoSalones();
                    foreach (PropertyInfo prop in typeof(CostoSalones).GetProperties())
                    {
                        object value = dr[prop.Name];
                        if (value == DBNull.Value) value = null;
