...
namespace DbEntidades.Operators
{
    public partial class SectoresEmpresaOperator
    {
        ...
        public static SectoresEmpresa GetOneByParameter(string campo, string valor)
        {
            if (!DbEntidades.Seguridad.Permiso("PermisoSectoresEmpresaBrowse")) throw new PermisoException();
            string columnas = string.Empty;
        ...
        columnas = columnas.Substring(0, columnas.Length - 2);
            DB db = new DB();
            DataTable dt = db.GetDataSet("select " + columnas + " from SectoresEmpresa where  " + campo + " = \'" + valor + "\'").Tables[0];
            SectoresEmpresa SectoresEmpresa = new SectoresEmpresa();
            if (dt.Rows.Count > 0)
            {
                foreach (PropertyInfo prop in typeof(SectoresEmpresa).GetProperties())
        ...
        public static List<SectoresEmpresa> GetAllByParameter(string campo, string valor)
            {
                if (!DbEntidades.Seguridad.Permiso("PermisoSectoresEmpresaBrowse")) throw new PermisoException();
                string columnas = string.Empty;
                ...
                if (tipo == "String")
                {
                    queryStr = "select " + columnas + " from SectoresEmpresa where " + campo + " = \'" + valor.ToString() + "\'";
                }
                else
                {
                    queryStr = "select " + columnas + " from SectoresEmpresa where " + campo + " = " + valor.ToString();
                ...
                foreach (DataRow dr in dt.AsEnumerable())
                {
                    ...
                    SectoresEmpresa entidad = new SectoresEmpresa();
                    foreach (PropertyInfo prop in typeof(SectoresEmpresa).GetProperties())
                    {
                        object value = dr[prop.Name];
                        if (value == DBNull.Value) value = null;
