...
namespace DbEntidades.Operators
{
    public partial class NombreFantasiaOperator
    {
        ...
        public static NombreFantasia GetOneByParameter(string campo, string valor)
        {
            if (!DbEntidades.Seguridad.Permiso("PermisoNombreFantasiaBrowse")) throw new PermisoException();
            string columnas = string.Empty;
        ...
        columnas = columnas.Substring(0, columnas.Length - 2);
            DB db = new DB();
            DataTable dt = db.GetDataSet("select " + columnas + " from NombreFantasia where  " + campo + " = \'" + valor + "\'").Tables[0];
            NombreFantasia NombreFantasia = new NombreFantasia();
            if (dt.Rows.Count > 0)
            {
                foreach (PropertyInfo prop in typeof(NombreFantasia).GetProperties())
        ...
        public static List<NombreFantasia> GetAllByParameter(string campo, string valor)
            {
                if (!DbEntidades.Seguridad.Permiso("PermisoNombreFantasiaBrowse")) throw new PermisoException();
                string columnas = string.Empty;
                ...
                if (tipo == "String")
                {
                    queryStr = "select " + columnas + " from NombreFantasia where " + campo + " = \'" + valor.ToString() + "\'";
                }
                else
                {
                    queryStr = "select " + columnas + " from NombreFantasia where " + campo + " = " + valor.ToString();
                ...
                foreach (DataRow dr in dt.AsEnumerable())
                {
                    ...
                    NombreFantasia entidad = new NombreFantasia();
                    foreach (PropertyInfo prop in typeof(NombreFantasia).GetProperties())
                    {
                        object value = dr[prop.Name];
                        if (value == DBNull.Value) value = null;
