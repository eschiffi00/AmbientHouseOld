...
namespace DbEntidades.Operators
{
    public partial class INVENTARIO_ProductoDepositoOperator
    {
        ...
        public static INVENTARIO_ProductoDeposito GetOneByParameter(string campo, string valor)
        {
            if (!DbEntidades.Seguridad.Permiso("PermisoINVENTARIO_ProductoDepositoBrowse")) throw new PermisoException();
            string columnas = string.Empty;
        ...
        columnas = columnas.Substring(0, columnas.Length - 2);
            DB db = new DB();
            DataTable dt = db.GetDataSet("select " + columnas + " from INVENTARIO_ProductoDeposito where  " + campo + " = \'" + valor + "\'").Tables[0];
            INVENTARIO_ProductoDeposito INVENTARIO_ProductoDeposito = new INVENTARIO_ProductoDeposito();
            if (dt.Rows.Count > 0)
            {
                foreach (PropertyInfo prop in typeof(INVENTARIO_ProductoDeposito).GetProperties())
        ...
        public static List<INVENTARIO_ProductoDeposito> GetAllByParameter(string campo, string valor)
            {
                if (!DbEntidades.Seguridad.Permiso("PermisoINVENTARIO_ProductoDepositoBrowse")) throw new PermisoException();
                string columnas = string.Empty;
                ...
                if (tipo == "String")
                {
                    queryStr = "select " + columnas + " from INVENTARIO_ProductoDeposito where " + campo + " = \'" + valor.ToString() + "\'";
                }
                else
                {
                    queryStr = "select " + columnas + " from INVENTARIO_ProductoDeposito where " + campo + " = " + valor.ToString();
                ...
                foreach (DataRow dr in dt.AsEnumerable())
                {
                    ...
                    INVENTARIO_ProductoDeposito entidad = new INVENTARIO_ProductoDeposito();
                    foreach (PropertyInfo prop in typeof(INVENTARIO_ProductoDeposito).GetProperties())
                    {
                        object value = dr[prop.Name];
                        if (value == DBNull.Value) value = null;
