...
namespace DbEntidades.Operators
{
    public partial class RubrosOperator
    {
        ...
        public static Rubros GetOneByParameter(string campo, string valor)
        {
            if (!DbEntidades.Seguridad.Permiso("PermisoRubrosBrowse")) throw new PermisoException();
            string columnas = string.Empty;
        ...
        columnas = columnas.Substring(0, columnas.Length - 2);
            DB db = new DB();
            DataTable dt = db.GetDataSet("select " + columnas + " from Rubros where  " + campo + " = \'" + valor + "\'").Tables[0];
            Rubros Rubros = new Rubros();
            if (dt.Rows.Count > 0)
            {
                foreach (PropertyInfo prop in typeof(Rubros).GetProperties())
        ...
        public static List<Rubros> GetAllByParameter(string campo, string valor)
            {
                if (!DbEntidades.Seguridad.Permiso("PermisoRubrosBrowse")) throw new PermisoException();
                string columnas = string.Empty;
                ...
                if (tipo == "String")
                {
                    queryStr = "select " + columnas + " from Rubros where " + campo + " = \'" + valor.ToString() + "\'";
                }
                else
                {
                    queryStr = "select " + columnas + " from Rubros where " + campo + " = " + valor.ToString();
