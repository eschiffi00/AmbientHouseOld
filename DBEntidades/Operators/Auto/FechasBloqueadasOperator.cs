...
namespace DbEntidades.Operators
{
    public partial class FechasBloqueadasOperator
    {
        ...
        public static FechasBloqueadas GetOneByParameter(string campo, string valor)
        {
            if (!DbEntidades.Seguridad.Permiso("PermisoFechasBloqueadasBrowse")) throw new PermisoException();
            string columnas = string.Empty;
        ...
        columnas = columnas.Substring(0, columnas.Length - 2);
            DB db = new DB();
            DataTable dt = db.GetDataSet("select " + columnas + " from FechasBloqueadas where  " + campo + " = \'" + valor + "\'").Tables[0];
            FechasBloqueadas FechasBloqueadas = new FechasBloqueadas();
            if (dt.Rows.Count > 0)
            {
                foreach (PropertyInfo prop in typeof(FechasBloqueadas).GetProperties())
        ...
        public static List<FechasBloqueadas> GetAllByParameter(string campo, string valor)
            {
                if (!DbEntidades.Seguridad.Permiso("PermisoFechasBloqueadasBrowse")) throw new PermisoException();
                string columnas = string.Empty;
                ...
                if (tipo == "String")
                {
                    queryStr = "select " + columnas + " from FechasBloqueadas where " + campo + " = \'" + valor.ToString() + "\'";
                }
                else
                {
                    queryStr = "select " + columnas + " from FechasBloqueadas where " + campo + " = " + valor.ToString();
                ...
                foreach (DataRow dr in dt.AsEnumerable())
                {
                    ...
                    FechasBloqueadas entidad = new FechasBloqueadas();
                    foreach (PropertyInfo prop in typeof(FechasBloqueadas).GetProperties())
                    {
                        object value = dr[prop.Name];
                        if (value == DBNull.Value) value = null;
