...
namespace DbEntidades.Operators
{
    public partial class ComprobantesProveedores_DetallesOperator
    {
        ...
        public static ComprobantesProveedores_Detalles GetOneByParameter(string campo, string valor)
        {
            if (!DbEntidades.Seguridad.Permiso("PermisoComprobantesProveedores_DetallesBrowse")) throw new PermisoException();
            string columnas = string.Empty;
        ...
        columnas = columnas.Substring(0, columnas.Length - 2);
            DB db = new DB();
            DataTable dt = db.GetDataSet("select " + columnas + " from ComprobantesProveedores_Detalles where  " + campo + " = \'" + valor + "\'").Tables[0];
            ComprobantesProveedores_Detalles ComprobantesProveedores_Detalles = new ComprobantesProveedores_Detalles();
            if (dt.Rows.Count > 0)
            {
                foreach (PropertyInfo prop in typeof(ComprobantesProveedores_Detalles).GetProperties())
        ...
        public static List<ComprobantesProveedores_Detalles> GetAllByParameter(string campo, string valor)
            {
                if (!DbEntidades.Seguridad.Permiso("PermisoComprobantesProveedores_DetallesBrowse")) throw new PermisoException();
                string columnas = string.Empty;
                ...
                if (tipo == "String")
                {
                    queryStr = "select " + columnas + " from ComprobantesProveedores_Detalles where " + campo + " = \'" + valor.ToString() + "\'";
                }
                else
                {
                    queryStr = "select " + columnas + " from ComprobantesProveedores_Detalles where " + campo + " = " + valor.ToString();
                ...
                foreach (DataRow dr in dt.AsEnumerable())
                {
                    ...
                    ComprobantesProveedores_Detalles entidad = new ComprobantesProveedores_Detalles();
                    foreach (PropertyInfo prop in typeof(ComprobantesProveedores_Detalles).GetProperties())
                    {
                        object value = dr[prop.Name];
                        if (value == DBNull.Value) value = null;
