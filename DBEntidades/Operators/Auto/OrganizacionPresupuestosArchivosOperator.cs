...
namespace DbEntidades.Operators
{
    public partial class OrganizacionPresupuestosArchivosOperator
    {
        ...
        public static OrganizacionPresupuestosArchivos GetOneByParameter(string campo, string valor)
        {
            if (!DbEntidades.Seguridad.Permiso("PermisoOrganizacionPresupuestosArchivosBrowse")) throw new PermisoException();
            string columnas = string.Empty;
        ...
        columnas = columnas.Substring(0, columnas.Length - 2);
            DB db = new DB();
            DataTable dt = db.GetDataSet("select " + columnas + " from OrganizacionPresupuestosArchivos where  " + campo + " = \'" + valor + "\'").Tables[0];
            OrganizacionPresupuestosArchivos OrganizacionPresupuestosArchivos = new OrganizacionPresupuestosArchivos();
            if (dt.Rows.Count > 0)
            {
                foreach (PropertyInfo prop in typeof(OrganizacionPresupuestosArchivos).GetProperties())
        ...
        public static List<OrganizacionPresupuestosArchivos> GetAllByParameter(string campo, string valor)
            {
                if (!DbEntidades.Seguridad.Permiso("PermisoOrganizacionPresupuestosArchivosBrowse")) throw new PermisoException();
                string columnas = string.Empty;
                ...
                if (tipo == "String")
                {
                    queryStr = "select " + columnas + " from OrganizacionPresupuestosArchivos where " + campo + " = \'" + valor.ToString() + "\'";
                }
                else
                {
                    queryStr = "select " + columnas + " from OrganizacionPresupuestosArchivos where " + campo + " = " + valor.ToString();
                ...
                foreach (DataRow dr in dt.AsEnumerable())
                {
                    ...
                    OrganizacionPresupuestosArchivos entidad = new OrganizacionPresupuestosArchivos();
                    foreach (PropertyInfo prop in typeof(OrganizacionPresupuestosArchivos).GetProperties())
                    {
                        object value = dr[prop.Name];
                        if (value == DBNull.Value) value = null;
