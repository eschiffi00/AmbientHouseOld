...
namespace DbEntidades.Operators
{
    public partial class ClientesBisOperator
    {
        ...
        public static ClientesBis GetOneByParameter(string campo, string valor)
        {
            if (!DbEntidades.Seguridad.Permiso("PermisoClientesBisBrowse")) throw new PermisoException();
            string columnas = string.Empty;
        ...
        columnas = columnas.Substring(0, columnas.Length - 2);
            DB db = new DB();
            DataTable dt = db.GetDataSet("select " + columnas + " from ClientesBis where  " + campo + " = \'" + valor + "\'").Tables[0];
            ClientesBis ClientesBis = new ClientesBis();
            if (dt.Rows.Count > 0)
            {
                foreach (PropertyInfo prop in typeof(ClientesBis).GetProperties())
        ...
        public static List<ClientesBis> GetAllByParameter(string campo, string valor)
            {
                if (!DbEntidades.Seguridad.Permiso("PermisoClientesBisBrowse")) throw new PermisoException();
                string columnas = string.Empty;
                ...
                if (tipo == "String")
                {
                    queryStr = "select " + columnas + " from ClientesBis where " + campo + " = \'" + valor.ToString() + "\'";
                }
                else
                {
                    queryStr = "select " + columnas + " from ClientesBis where " + campo + " = " + valor.ToString();
                ...
                foreach (DataRow dr in dt.AsEnumerable())
                {
                    ...
                    ClientesBis entidad = new ClientesBis();
                    foreach (PropertyInfo prop in typeof(ClientesBis).GetProperties())
                    {
                        object value = dr[prop.Name];
                        if (value == DBNull.Value) value = null;
