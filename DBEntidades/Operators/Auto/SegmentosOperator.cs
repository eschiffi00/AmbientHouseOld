...
namespace DbEntidades.Operators
{
    public partial class SegmentosOperator
    {
        ...
        public static Segmentos GetOneByParameter(string campo, string valor)
        {
            if (!DbEntidades.Seguridad.Permiso("PermisoSegmentosBrowse")) throw new PermisoException();
            string columnas = string.Empty;
        ...
        columnas = columnas.Substring(0, columnas.Length - 2);
            DB db = new DB();
            DataTable dt = db.GetDataSet("select " + columnas + " from Segmentos where  " + campo + " = \'" + valor + "\'").Tables[0];
            Segmentos Segmentos = new Segmentos();
            if (dt.Rows.Count > 0)
            {
                foreach (PropertyInfo prop in typeof(Segmentos).GetProperties())
        ...
        public static List<Segmentos> GetAllByParameter(string campo, string valor)
            {
                if (!DbEntidades.Seguridad.Permiso("PermisoSegmentosBrowse")) throw new PermisoException();
                string columnas = string.Empty;
                ...
                if (tipo == "String")
                {
                    queryStr = "select " + columnas + " from Segmentos where " + campo + " = \'" + valor.ToString() + "\'";
                }
                else
                {
                    queryStr = "select " + columnas + " from Segmentos where " + campo + " = " + valor.ToString();
