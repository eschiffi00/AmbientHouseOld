...
namespace DbEntidades.Operators
{
    public partial class TipoCateringAdicionalOperator
    {
        ...
        public static TipoCateringAdicional GetOneByParameter(string campo, string valor)
        {
            if (!DbEntidades.Seguridad.Permiso("PermisoTipoCateringAdicionalBrowse")) throw new PermisoException();
            string columnas = string.Empty;
        ...
        columnas = columnas.Substring(0, columnas.Length - 2);
            DB db = new DB();
            DataTable dt = db.GetDataSet("select " + columnas + " from TipoCateringAdicional where  " + campo + " = \'" + valor + "\'").Tables[0];
            TipoCateringAdicional TipoCateringAdicional = new TipoCateringAdicional();
            if (dt.Rows.Count > 0)
            {
                foreach (PropertyInfo prop in typeof(TipoCateringAdicional).GetProperties())
        ...
        public static List<TipoCateringAdicional> GetAllByParameter(string campo, string valor)
            {
                if (!DbEntidades.Seguridad.Permiso("PermisoTipoCateringAdicionalBrowse")) throw new PermisoException();
                string columnas = string.Empty;
                ...
                if (tipo == "String")
                {
                    queryStr = "select " + columnas + " from TipoCateringAdicional where " + campo + " = \'" + valor.ToString() + "\'";
                }
                else
                {
                    queryStr = "select " + columnas + " from TipoCateringAdicional where " + campo + " = " + valor.ToString();
                ...
                foreach (DataRow dr in dt.AsEnumerable())
                {
                    ...
                    TipoCateringAdicional entidad = new TipoCateringAdicional();
                    foreach (PropertyInfo prop in typeof(TipoCateringAdicional).GetProperties())
                    {
                        object value = dr[prop.Name];
                        if (value == DBNull.Value) value = null;
