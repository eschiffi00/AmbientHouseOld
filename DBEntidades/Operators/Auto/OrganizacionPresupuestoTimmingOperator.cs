...
namespace DbEntidades.Operators
{
    public partial class OrganizacionPresupuestoTimmingOperator
    {
        ...
        public static OrganizacionPresupuestoTimming GetOneByParameter(string campo, string valor)
        {
            if (!DbEntidades.Seguridad.Permiso("PermisoOrganizacionPresupuestoTimmingBrowse")) throw new PermisoException();
            string columnas = string.Empty;
        ...
        columnas = columnas.Substring(0, columnas.Length - 2);
            DB db = new DB();
            DataTable dt = db.GetDataSet("select " + columnas + " from OrganizacionPresupuestoTimming where  " + campo + " = \'" + valor + "\'").Tables[0];
            OrganizacionPresupuestoTimming OrganizacionPresupuestoTimming = new OrganizacionPresupuestoTimming();
            if (dt.Rows.Count > 0)
            {
                foreach (PropertyInfo prop in typeof(OrganizacionPresupuestoTimming).GetProperties())
        ...
        public static List<OrganizacionPresupuestoTimming> GetAllByParameter(string campo, string valor)
            {
                if (!DbEntidades.Seguridad.Permiso("PermisoOrganizacionPresupuestoTimmingBrowse")) throw new PermisoException();
                string columnas = string.Empty;
                ...
                if (tipo == "String")
                {
                    queryStr = "select " + columnas + " from OrganizacionPresupuestoTimming where " + campo + " = \'" + valor.ToString() + "\'";
                }
                else
                {
                    queryStr = "select " + columnas + " from OrganizacionPresupuestoTimming where " + campo + " = " + valor.ToString();
                ...
                foreach (DataRow dr in dt.AsEnumerable())
                {
                    ...
                    OrganizacionPresupuestoTimming entidad = new OrganizacionPresupuestoTimming();
                    foreach (PropertyInfo prop in typeof(OrganizacionPresupuestoTimming).GetProperties())
                    {
                        object value = dr[prop.Name];
                        if (value == DBNull.Value) value = null;
