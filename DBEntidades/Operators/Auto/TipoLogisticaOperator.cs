...
namespace DbEntidades.Operators
{
    public partial class TipoLogisticaOperator
    {
        ...
        public static TipoLogistica GetOneByParameter(string campo, string valor)
        {
            if (!DbEntidades.Seguridad.Permiso("PermisoTipoLogisticaBrowse")) throw new PermisoException();
            string columnas = string.Empty;
        ...
        columnas = columnas.Substring(0, columnas.Length - 2);
            DB db = new DB();
            DataTable dt = db.GetDataSet("select " + columnas + " from TipoLogistica where  " + campo + " = \'" + valor + "\'").Tables[0];
            TipoLogistica TipoLogistica = new TipoLogistica();
            if (dt.Rows.Count > 0)
            {
                foreach (PropertyInfo prop in typeof(TipoLogistica).GetProperties())
        ...
        public static List<TipoLogistica> GetAllByParameter(string campo, string valor)
            {
                if (!DbEntidades.Seguridad.Permiso("PermisoTipoLogisticaBrowse")) throw new PermisoException();
                string columnas = string.Empty;
                ...
                if (tipo == "String")
                {
                    queryStr = "select " + columnas + " from TipoLogistica where " + campo + " = \'" + valor.ToString() + "\'";
                }
                else
                {
                    queryStr = "select " + columnas + " from TipoLogistica where " + campo + " = " + valor.ToString();
                ...
                foreach (DataRow dr in dt.AsEnumerable())
                {
                    ...
                    TipoLogistica entidad = new TipoLogistica();
                    foreach (PropertyInfo prop in typeof(TipoLogistica).GetProperties())
                    {
                        object value = dr[prop.Name];
                        if (value == DBNull.Value) value = null;
