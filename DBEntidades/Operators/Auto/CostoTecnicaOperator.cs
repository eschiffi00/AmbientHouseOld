...
namespace DbEntidades.Operators
{
    public partial class CostoTecnicaOperator
    {
        ...
        public static CostoTecnica GetOneByParameter(string campo, string valor)
        {
            if (!DbEntidades.Seguridad.Permiso("PermisoCostoTecnicaBrowse")) throw new PermisoException();
            string columnas = string.Empty;
        ...
        columnas = columnas.Substring(0, columnas.Length - 2);
            DB db = new DB();
            DataTable dt = db.GetDataSet("select " + columnas + " from CostoTecnica where  " + campo + " = \'" + valor + "\'").Tables[0];
            CostoTecnica CostoTecnica = new CostoTecnica();
            if (dt.Rows.Count > 0)
            {
                foreach (PropertyInfo prop in typeof(CostoTecnica).GetProperties())
        ...
        public static List<CostoTecnica> GetAllByParameter(string campo, string valor)
            {
                if (!DbEntidades.Seguridad.Permiso("PermisoCostoTecnicaBrowse")) throw new PermisoException();
                string columnas = string.Empty;
                ...
                if (tipo == "String")
                {
                    queryStr = "select " + columnas + " from CostoTecnica where " + campo + " = \'" + valor.ToString() + "\'";
                }
                else
                {
                    queryStr = "select " + columnas + " from CostoTecnica where " + campo + " = " + valor.ToString();
                ...
                foreach (DataRow dr in dt.AsEnumerable())
                {
                    ...
                    CostoTecnica entidad = new CostoTecnica();
                    foreach (PropertyInfo prop in typeof(CostoTecnica).GetProperties())
                    {
                        object value = dr[prop.Name];
                        if (value == DBNull.Value) value = null;
