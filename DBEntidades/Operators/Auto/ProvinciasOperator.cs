...
namespace DbEntidades.Operators
{
    public partial class ProvinciasOperator
    {
        ...
        public static Provincias GetOneByParameter(string campo, string valor)
        {
            if (!DbEntidades.Seguridad.Permiso("PermisoProvinciasBrowse")) throw new PermisoException();
            string columnas = string.Empty;
        ...
        columnas = columnas.Substring(0, columnas.Length - 2);
            DB db = new DB();
            DataTable dt = db.GetDataSet("select " + columnas + " from Provincias where  " + campo + " = \'" + valor + "\'").Tables[0];
            Provincias Provincias = new Provincias();
            if (dt.Rows.Count > 0)
            {
                foreach (PropertyInfo prop in typeof(Provincias).GetProperties())
        ...
        public static List<Provincias> GetAllByParameter(string campo, string valor)
            {
                if (!DbEntidades.Seguridad.Permiso("PermisoProvinciasBrowse")) throw new PermisoException();
                string columnas = string.Empty;
                ...
                if (tipo == "String")
                {
                    queryStr = "select " + columnas + " from Provincias where " + campo + " = \'" + valor.ToString() + "\'";
                }
                else
                {
                    queryStr = "select " + columnas + " from Provincias where " + campo + " = " + valor.ToString();
                ...
                foreach (DataRow dr in dt.AsEnumerable())
                {
                    ...
                    Provincias entidad = new Provincias();
                    foreach (PropertyInfo prop in typeof(Provincias).GetProperties())
                    {
                        object value = dr[prop.Name];
                        if (value == DBNull.Value) value = null;
