...
namespace DbEntidades.Operators
{
    public partial class CostoCanonOperator
    {
        ...
        public static CostoCanon GetOneByParameter(string campo, string valor)
        {
            if (!DbEntidades.Seguridad.Permiso("PermisoCostoCanonBrowse")) throw new PermisoException();
            string columnas = string.Empty;
        ...
        columnas = columnas.Substring(0, columnas.Length - 2);
            DB db = new DB();
            DataTable dt = db.GetDataSet("select " + columnas + " from CostoCanon where  " + campo + " = \'" + valor + "\'").Tables[0];
            CostoCanon CostoCanon = new CostoCanon();
            if (dt.Rows.Count > 0)
            {
                foreach (PropertyInfo prop in typeof(CostoCanon).GetProperties())
        ...
        public static List<CostoCanon> GetAllByParameter(string campo, string valor)
            {
                if (!DbEntidades.Seguridad.Permiso("PermisoCostoCanonBrowse")) throw new PermisoException();
                string columnas = string.Empty;
                ...
                if (tipo == "String")
                {
                    queryStr = "select " + columnas + " from CostoCanon where " + campo + " = \'" + valor.ToString() + "\'";
                }
                else
                {
                    queryStr = "select " + columnas + " from CostoCanon where " + campo + " = " + valor.ToString();
                ...
                foreach (DataRow dr in dt.AsEnumerable())
                {
                    ...
                    CostoCanon entidad = new CostoCanon();
                    foreach (PropertyInfo prop in typeof(CostoCanon).GetProperties())
                    {
                        object value = dr[prop.Name];
                        if (value == DBNull.Value) value = null;
