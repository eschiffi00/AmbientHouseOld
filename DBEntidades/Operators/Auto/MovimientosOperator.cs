...
namespace DbEntidades.Operators
{
    public partial class MovimientosOperator
    {
        ...
        public static Movimientos GetOneByParameter(string campo, string valor)
        {
            if (!DbEntidades.Seguridad.Permiso("PermisoMovimientosBrowse")) throw new PermisoException();
            string columnas = string.Empty;
        ...
        columnas = columnas.Substring(0, columnas.Length - 2);
            DB db = new DB();
            DataTable dt = db.GetDataSet("select " + columnas + " from Movimientos where  " + campo + " = \'" + valor + "\'").Tables[0];
            Movimientos Movimientos = new Movimientos();
            if (dt.Rows.Count > 0)
            {
                foreach (PropertyInfo prop in typeof(Movimientos).GetProperties())
        ...
        public static List<Movimientos> GetAllByParameter(string campo, string valor)
            {
                if (!DbEntidades.Seguridad.Permiso("PermisoMovimientosBrowse")) throw new PermisoException();
                string columnas = string.Empty;
                ...
                if (tipo == "String")
                {
                    queryStr = "select " + columnas + " from Movimientos where " + campo + " = \'" + valor.ToString() + "\'";
                }
                else
                {
                    queryStr = "select " + columnas + " from Movimientos where " + campo + " = " + valor.ToString();
