...
namespace DbEntidades.Operators
{
    public partial class CostoAdicionalesOperator
    {
        ...
        public static CostoAdicionales GetOneByParameter(string campo, string valor)
        {
            if (!DbEntidades.Seguridad.Permiso("PermisoCostoAdicionalesBrowse")) throw new PermisoException();
            string columnas = string.Empty;
        ...
        columnas = columnas.Substring(0, columnas.Length - 2);
            DB db = new DB();
            DataTable dt = db.GetDataSet("select " + columnas + " from CostoAdicionales where  " + campo + " = \'" + valor + "\'").Tables[0];
            CostoAdicionales CostoAdicionales = new CostoAdicionales();
            if (dt.Rows.Count > 0)
            {
                foreach (PropertyInfo prop in typeof(CostoAdicionales).GetProperties())
        ...
        public static List<CostoAdicionales> GetAllByParameter(string campo, string valor)
            {
                if (!DbEntidades.Seguridad.Permiso("PermisoCostoAdicionalesBrowse")) throw new PermisoException();
                string columnas = string.Empty;
                ...
                if (tipo == "String")
                {
                    queryStr = "select " + columnas + " from CostoAdicionales where " + campo + " = \'" + valor.ToString() + "\'";
                }
                else
                {
                    queryStr = "select " + columnas + " from CostoAdicionales where " + campo + " = " + valor.ToString();
                ...
                foreach (DataRow dr in dt.AsEnumerable())
                {
                    ...
                    CostoAdicionales entidad = new CostoAdicionales();
                    foreach (PropertyInfo prop in typeof(CostoAdicionales).GetProperties())
                    {
                        object value = dr[prop.Name];
                        if (value == DBNull.Value) value = null;
