...
namespace DbEntidades.Operators
{
    public partial class ProductosCateringItemsOperator
    {
        ...
        public static ProductosCateringItems GetOneByParameter(string campo, string valor)
        {
            if (!DbEntidades.Seguridad.Permiso("PermisoProductosCateringItemsBrowse")) throw new PermisoException();
            string columnas = string.Empty;
        ...
        columnas = columnas.Substring(0, columnas.Length - 2);
            DB db = new DB();
            DataTable dt = db.GetDataSet("select " + columnas + " from ProductosCateringItems where  " + campo + " = \'" + valor + "\'").Tables[0];
            ProductosCateringItems ProductosCateringItems = new ProductosCateringItems();
            if (dt.Rows.Count > 0)
            {
                foreach (PropertyInfo prop in typeof(ProductosCateringItems).GetProperties())
        ...
        public static List<ProductosCateringItems> GetAllByParameter(string campo, string valor)
            {
                if (!DbEntidades.Seguridad.Permiso("PermisoProductosCateringItemsBrowse")) throw new PermisoException();
                string columnas = string.Empty;
                ...
                if (tipo == "String")
                {
                    queryStr = "select " + columnas + " from ProductosCateringItems where " + campo + " = \'" + valor.ToString() + "\'";
                }
                else
                {
                    queryStr = "select " + columnas + " from ProductosCateringItems where " + campo + " = " + valor.ToString();
                ...
                foreach (DataRow dr in dt.AsEnumerable())
                {
                    ...
                    ProductosCateringItems entidad = new ProductosCateringItems();
                    foreach (PropertyInfo prop in typeof(ProductosCateringItems).GetProperties())
                    {
                        object value = dr[prop.Name];
                        if (value == DBNull.Value) value = null;
