...
namespace DbEntidades.Operators
{
    public partial class BloqueoOperator
    {
        ...
        public static Bloqueo GetOneByParameter(string campo, string valor)
        {
            if (!DbEntidades.Seguridad.Permiso("PermisoBloqueoBrowse")) throw new PermisoException();
            string columnas = string.Empty;
        ...
        columnas = columnas.Substring(0, columnas.Length - 2);
            DB db = new DB();
            DataTable dt = db.GetDataSet("select " + columnas + " from Bloqueo where  " + campo + " = \'" + valor + "\'").Tables[0];
            Bloqueo Bloqueo = new Bloqueo();
            if (dt.Rows.Count > 0)
            {
                foreach (PropertyInfo prop in typeof(Bloqueo).GetProperties())
        ...
        public static List<Bloqueo> GetAllByParameter(string campo, string valor)
            {
                if (!DbEntidades.Seguridad.Permiso("PermisoBloqueoBrowse")) throw new PermisoException();
                string columnas = string.Empty;
                ...
                if (tipo == "String")
                {
                    queryStr = "select " + columnas + " from Bloqueo where " + campo + " = \'" + valor.ToString() + "\'";
                }
                else
                {
                    queryStr = "select " + columnas + " from Bloqueo where " + campo + " = " + valor.ToString();
