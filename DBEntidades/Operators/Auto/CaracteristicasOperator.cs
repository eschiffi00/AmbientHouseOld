...
namespace DbEntidades.Operators
{
    public partial class CaracteristicasOperator
    {
        ...
        public static Caracteristicas GetOneByParameter(string campo, string valor)
        {
            if (!DbEntidades.Seguridad.Permiso("PermisoCaracteristicasBrowse")) throw new PermisoException();
            string columnas = string.Empty;
        ...
        columnas = columnas.Substring(0, columnas.Length - 2);
            DB db = new DB();
            DataTable dt = db.GetDataSet("select " + columnas + " from Caracteristicas where  " + campo + " = \'" + valor + "\'").Tables[0];
            Caracteristicas Caracteristicas = new Caracteristicas();
            if (dt.Rows.Count > 0)
            {
                foreach (PropertyInfo prop in typeof(Caracteristicas).GetProperties())
        ...
        public static List<Caracteristicas> GetAllByParameter(string campo, string valor)
            {
                if (!DbEntidades.Seguridad.Permiso("PermisoCaracteristicasBrowse")) throw new PermisoException();
                string columnas = string.Empty;
                ...
                if (tipo == "String")
                {
                    queryStr = "select " + columnas + " from Caracteristicas where " + campo + " = \'" + valor.ToString() + "\'";
                }
                else
                {
                    queryStr = "select " + columnas + " from Caracteristicas where " + campo + " = " + valor.ToString();
                ...
                foreach (DataRow dr in dt.AsEnumerable())
                {
                    ...
                    Caracteristicas entidad = new Caracteristicas();
                    foreach (PropertyInfo prop in typeof(Caracteristicas).GetProperties())
                    {
                        object value = dr[prop.Name];
                        if (value == DBNull.Value) value = null;
