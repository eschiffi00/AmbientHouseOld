...
namespace DbEntidades.Operators
{
    public partial class ProductosOperator
    {
        ...
        public static Productos GetOneByParameter(string campo, string valor)
        {
            if (!DbEntidades.Seguridad.Permiso("PermisoProductosBrowse")) throw new PermisoException();
            string columnas = string.Empty;
        ...
        columnas = columnas.Substring(0, columnas.Length - 2);
            DB db = new DB();
            DataTable dt = db.GetDataSet("select " + columnas + " from Productos where  " + campo + " = \'" + valor + "\'").Tables[0];
            Productos Productos = new Productos();
            if (dt.Rows.Count > 0)
            {
                foreach (PropertyInfo prop in typeof(Productos).GetProperties())
        ...
        public static List<Productos> GetAllByParameter(string campo, string valor)
            {
                if (!DbEntidades.Seguridad.Permiso("PermisoProductosBrowse")) throw new PermisoException();
                string columnas = string.Empty;
                ...
                if (tipo == "String")
                {
                    queryStr = "select " + columnas + " from Productos where " + campo + " = \'" + valor.ToString() + "\'";
                }
                else
                {
                    queryStr = "select " + columnas + " from Productos where " + campo + " = " + valor.ToString();
