...
namespace DbEntidades.Operators
{
    public partial class MonedasOperator
    {
        ...
        public static Monedas GetOneByParameter(string campo, string valor)
        {
            if (!DbEntidades.Seguridad.Permiso("PermisoMonedasBrowse")) throw new PermisoException();
            string columnas = string.Empty;
        ...
        columnas = columnas.Substring(0, columnas.Length - 2);
            DB db = new DB();
            DataTable dt = db.GetDataSet("select " + columnas + " from Monedas where  " + campo + " = \'" + valor + "\'").Tables[0];
            Monedas Monedas = new Monedas();
            if (dt.Rows.Count > 0)
            {
                foreach (PropertyInfo prop in typeof(Monedas).GetProperties())
        ...
        public static List<Monedas> GetAllByParameter(string campo, string valor)
            {
                if (!DbEntidades.Seguridad.Permiso("PermisoMonedasBrowse")) throw new PermisoException();
                string columnas = string.Empty;
                ...
                if (tipo == "String")
                {
                    queryStr = "select " + columnas + " from Monedas where " + campo + " = \'" + valor.ToString() + "\'";
                }
                else
                {
                    queryStr = "select " + columnas + " from Monedas where " + campo + " = " + valor.ToString();
