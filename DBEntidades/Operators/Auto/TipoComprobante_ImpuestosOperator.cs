...
namespace DbEntidades.Operators
{
    public partial class TipoComprobante_ImpuestosOperator
    {
        ...
        public static TipoComprobante_Impuestos GetOneByParameter(string campo, string valor)
        {
            if (!DbEntidades.Seguridad.Permiso("PermisoTipoComprobante_ImpuestosBrowse")) throw new PermisoException();
            string columnas = string.Empty;
        ...
        columnas = columnas.Substring(0, columnas.Length - 2);
            DB db = new DB();
            DataTable dt = db.GetDataSet("select " + columnas + " from TipoComprobante_Impuestos where  " + campo + " = \'" + valor + "\'").Tables[0];
            TipoComprobante_Impuestos TipoComprobante_Impuestos = new TipoComprobante_Impuestos();
            if (dt.Rows.Count > 0)
            {
                foreach (PropertyInfo prop in typeof(TipoComprobante_Impuestos).GetProperties())
        ...
        public static List<TipoComprobante_Impuestos> GetAllByParameter(string campo, string valor)
            {
                if (!DbEntidades.Seguridad.Permiso("PermisoTipoComprobante_ImpuestosBrowse")) throw new PermisoException();
                string columnas = string.Empty;
                ...
                if (tipo == "String")
                {
                    queryStr = "select " + columnas + " from TipoComprobante_Impuestos where " + campo + " = \'" + valor.ToString() + "\'";
                }
                else
                {
                    queryStr = "select " + columnas + " from TipoComprobante_Impuestos where " + campo + " = " + valor.ToString();
                ...
                foreach (DataRow dr in dt.AsEnumerable())
                {
                    ...
                    TipoComprobante_Impuestos entidad = new TipoComprobante_Impuestos();
                    foreach (PropertyInfo prop in typeof(TipoComprobante_Impuestos).GetProperties())
                    {
                        object value = dr[prop.Name];
                        if (value == DBNull.Value) value = null;
