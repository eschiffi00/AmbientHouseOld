...
namespace DbEntidades.Operators
{
    public partial class ReciboEventoPresupuestoOperator
    {
        ...
        public static ReciboEventoPresupuesto GetOneByParameter(string campo, string valor)
        {
            if (!DbEntidades.Seguridad.Permiso("PermisoReciboEventoPresupuestoBrowse")) throw new PermisoException();
            string columnas = string.Empty;
        ...
        columnas = columnas.Substring(0, columnas.Length - 2);
            DB db = new DB();
            DataTable dt = db.GetDataSet("select " + columnas + " from ReciboEventoPresupuesto where  " + campo + " = \'" + valor + "\'").Tables[0];
            ReciboEventoPresupuesto ReciboEventoPresupuesto = new ReciboEventoPresupuesto();
            if (dt.Rows.Count > 0)
            {
                foreach (PropertyInfo prop in typeof(ReciboEventoPresupuesto).GetProperties())
        ...
        public static List<ReciboEventoPresupuesto> GetAllByParameter(string campo, string valor)
            {
                if (!DbEntidades.Seguridad.Permiso("PermisoReciboEventoPresupuestoBrowse")) throw new PermisoException();
                string columnas = string.Empty;
                ...
                if (tipo == "String")
                {
                    queryStr = "select " + columnas + " from ReciboEventoPresupuesto where " + campo + " = \'" + valor.ToString() + "\'";
                }
                else
                {
                    queryStr = "select " + columnas + " from ReciboEventoPresupuesto where " + campo + " = " + valor.ToString();
                ...
                foreach (DataRow dr in dt.AsEnumerable())
                {
                    ...
                    ReciboEventoPresupuesto entidad = new ReciboEventoPresupuesto();
                    foreach (PropertyInfo prop in typeof(ReciboEventoPresupuesto).GetProperties())
                    {
                        object value = dr[prop.Name];
                        if (value == DBNull.Value) value = null;
