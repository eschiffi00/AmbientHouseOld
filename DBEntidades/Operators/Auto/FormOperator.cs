...
namespace DbEntidades.Operators
{
    public partial class FormOperator
    {
        ...
        public static Form GetOneByParameter(string campo, string valor)
        {
            if (!DbEntidades.Seguridad.Permiso("PermisoFormBrowse")) throw new PermisoException();
            string columnas = string.Empty;
        ...
        columnas = columnas.Substring(0, columnas.Length - 2);
            DB db = new DB();
            DataTable dt = db.GetDataSet("select " + columnas + " from Form where  " + campo + " = \'" + valor + "\'").Tables[0];
            Form Form = new Form();
            if (dt.Rows.Count > 0)
            {
                foreach (PropertyInfo prop in typeof(Form).GetProperties())
        ...
        public static List<Form> GetAllByParameter(string campo, string valor)
            {
                if (!DbEntidades.Seguridad.Permiso("PermisoFormBrowse")) throw new PermisoException();
                string columnas = string.Empty;
                ...
                if (tipo == "String")
                {
                    queryStr = "select " + columnas + " from Form where " + campo + " = \'" + valor.ToString() + "\'";
                }
                else
                {
                    queryStr = "select " + columnas + " from Form where " + campo + " = " + valor.ToString();
