...
namespace DbEntidades.Operators
{
    public partial class CostoAmbientacionOperator
    {
        ...
        public static CostoAmbientacion GetOneByParameter(string campo, string valor)
        {
            if (!DbEntidades.Seguridad.Permiso("PermisoCostoAmbientacionBrowse")) throw new PermisoException();
            string columnas = string.Empty;
        ...
        columnas = columnas.Substring(0, columnas.Length - 2);
            DB db = new DB();
            DataTable dt = db.GetDataSet("select " + columnas + " from CostoAmbientacion where  " + campo + " = \'" + valor + "\'").Tables[0];
            CostoAmbientacion CostoAmbientacion = new CostoAmbientacion();
            if (dt.Rows.Count > 0)
            {
                foreach (PropertyInfo prop in typeof(CostoAmbientacion).GetProperties())
        ...
        public static List<CostoAmbientacion> GetAllByParameter(string campo, string valor)
            {
                if (!DbEntidades.Seguridad.Permiso("PermisoCostoAmbientacionBrowse")) throw new PermisoException();
                string columnas = string.Empty;
                ...
                if (tipo == "String")
                {
                    queryStr = "select " + columnas + " from CostoAmbientacion where " + campo + " = \'" + valor.ToString() + "\'";
                }
                else
                {
                    queryStr = "select " + columnas + " from CostoAmbientacion where " + campo + " = " + valor.ToString();
                ...
                foreach (DataRow dr in dt.AsEnumerable())
                {
                    ...
                    CostoAmbientacion entidad = new CostoAmbientacion();
                    foreach (PropertyInfo prop in typeof(CostoAmbientacion).GetProperties())
                    {
                        object value = dr[prop.Name];
                        if (value == DBNull.Value) value = null;
