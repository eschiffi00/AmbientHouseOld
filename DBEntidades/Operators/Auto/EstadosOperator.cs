...
namespace DbEntidades.Operators
{
    public partial class EstadosOperator
    {
        ...
        public static Estados GetOneByParameter(string campo, string valor)
        {
            if (!DbEntidades.Seguridad.Permiso("PermisoEstadosBrowse")) throw new PermisoException();
            string columnas = string.Empty;
        ...
        columnas = columnas.Substring(0, columnas.Length - 2);
            DB db = new DB();
            DataTable dt = db.GetDataSet("select " + columnas + " from Estados where  " + campo + " = \'" + valor + "\'").Tables[0];
            Estados Estados = new Estados();
            if (dt.Rows.Count > 0)
            {
                foreach (PropertyInfo prop in typeof(Estados).GetProperties())
        ...
        public static List<Estados> GetAllByParameter(string campo, string valor)
            {
                if (!DbEntidades.Seguridad.Permiso("PermisoEstadosBrowse")) throw new PermisoException();
                string columnas = string.Empty;
                ...
                if (tipo == "String")
                {
                    queryStr = "select " + columnas + " from Estados where " + campo + " = \'" + valor.ToString() + "\'";
                }
                else
                {
                    queryStr = "select " + columnas + " from Estados where " + campo + " = " + valor.ToString();
