...
namespace DbEntidades.Operators
{
    public partial class CategoriasArchivosOperator
    {
        ...
        public static CategoriasArchivos GetOneByParameter(string campo, string valor)
        {
            if (!DbEntidades.Seguridad.Permiso("PermisoCategoriasArchivosBrowse")) throw new PermisoException();
            string columnas = string.Empty;
        ...
        columnas = columnas.Substring(0, columnas.Length - 2);
            DB db = new DB();
            DataTable dt = db.GetDataSet("select " + columnas + " from CategoriasArchivos where  " + campo + " = \'" + valor + "\'").Tables[0];
            CategoriasArchivos CategoriasArchivos = new CategoriasArchivos();
            if (dt.Rows.Count > 0)
            {
                foreach (PropertyInfo prop in typeof(CategoriasArchivos).GetProperties())
        ...
        public static List<CategoriasArchivos> GetAllByParameter(string campo, string valor)
            {
                if (!DbEntidades.Seguridad.Permiso("PermisoCategoriasArchivosBrowse")) throw new PermisoException();
                string columnas = string.Empty;
                ...
                if (tipo == "String")
                {
                    queryStr = "select " + columnas + " from CategoriasArchivos where " + campo + " = \'" + valor.ToString() + "\'";
                }
                else
                {
                    queryStr = "select " + columnas + " from CategoriasArchivos where " + campo + " = " + valor.ToString();
                ...
                foreach (DataRow dr in dt.AsEnumerable())
                {
                    ...
                    CategoriasArchivos entidad = new CategoriasArchivos();
                    foreach (PropertyInfo prop in typeof(CategoriasArchivos).GetProperties())
                    {
                        object value = dr[prop.Name];
                        if (value == DBNull.Value) value = null;
