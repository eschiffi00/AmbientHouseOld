...
namespace DbEntidades.Operators
{
    public partial class TipoCateringFamiliaOperator
    {
        ...
        public static TipoCateringFamilia GetOneByParameter(string campo, string valor)
        {
            if (!DbEntidades.Seguridad.Permiso("PermisoTipoCateringFamiliaBrowse")) throw new PermisoException();
            string columnas = string.Empty;
        ...
        columnas = columnas.Substring(0, columnas.Length - 2);
            DB db = new DB();
            DataTable dt = db.GetDataSet("select " + columnas + " from TipoCateringFamilia where  " + campo + " = \'" + valor + "\'").Tables[0];
            TipoCateringFamilia TipoCateringFamilia = new TipoCateringFamilia();
            if (dt.Rows.Count > 0)
            {
                foreach (PropertyInfo prop in typeof(TipoCateringFamilia).GetProperties())
        ...
        public static List<TipoCateringFamilia> GetAllByParameter(string campo, string valor)
            {
                if (!DbEntidades.Seguridad.Permiso("PermisoTipoCateringFamiliaBrowse")) throw new PermisoException();
                string columnas = string.Empty;
                ...
                if (tipo == "String")
                {
                    queryStr = "select " + columnas + " from TipoCateringFamilia where " + campo + " = \'" + valor.ToString() + "\'";
                }
                else
                {
                    queryStr = "select " + columnas + " from TipoCateringFamilia where " + campo + " = " + valor.ToString();
                ...
                foreach (DataRow dr in dt.AsEnumerable())
                {
                    ...
                    TipoCateringFamilia entidad = new TipoCateringFamilia();
                    foreach (PropertyInfo prop in typeof(TipoCateringFamilia).GetProperties())
                    {
                        object value = dr[prop.Name];
                        if (value == DBNull.Value) value = null;
