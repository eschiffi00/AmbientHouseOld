...
namespace DbEntidades.Operators
{
    public partial class RecibosOperator
    {
        ...
        public static Recibos GetOneByParameter(string campo, string valor)
        {
            if (!DbEntidades.Seguridad.Permiso("PermisoRecibosBrowse")) throw new PermisoException();
            string columnas = string.Empty;
        ...
        columnas = columnas.Substring(0, columnas.Length - 2);
            DB db = new DB();
            DataTable dt = db.GetDataSet("select " + columnas + " from Recibos where  " + campo + " = \'" + valor + "\'").Tables[0];
            Recibos Recibos = new Recibos();
            if (dt.Rows.Count > 0)
            {
                foreach (PropertyInfo prop in typeof(Recibos).GetProperties())
        ...
        public static List<Recibos> GetAllByParameter(string campo, string valor)
            {
                if (!DbEntidades.Seguridad.Permiso("PermisoRecibosBrowse")) throw new PermisoException();
                string columnas = string.Empty;
                ...
                if (tipo == "String")
                {
                    queryStr = "select " + columnas + " from Recibos where " + campo + " = \'" + valor.ToString() + "\'";
                }
                else
                {
                    queryStr = "select " + columnas + " from Recibos where " + campo + " = " + valor.ToString();
