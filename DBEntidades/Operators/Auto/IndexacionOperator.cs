...
namespace DbEntidades.Operators
{
    public partial class IndexacionOperator
    {
        ...
        public static Indexacion GetOneByParameter(string campo, string valor)
        {
            if (!DbEntidades.Seguridad.Permiso("PermisoIndexacionBrowse")) throw new PermisoException();
            string columnas = string.Empty;
        ...
        columnas = columnas.Substring(0, columnas.Length - 2);
            DB db = new DB();
            DataTable dt = db.GetDataSet("select " + columnas + " from Indexacion where  " + campo + " = \'" + valor + "\'").Tables[0];
            Indexacion Indexacion = new Indexacion();
            if (dt.Rows.Count > 0)
            {
                foreach (PropertyInfo prop in typeof(Indexacion).GetProperties())
        ...
        public static List<Indexacion> GetAllByParameter(string campo, string valor)
            {
                if (!DbEntidades.Seguridad.Permiso("PermisoIndexacionBrowse")) throw new PermisoException();
                string columnas = string.Empty;
                ...
                if (tipo == "String")
                {
                    queryStr = "select " + columnas + " from Indexacion where " + campo + " = \'" + valor.ToString() + "\'";
                }
                else
                {
                    queryStr = "select " + columnas + " from Indexacion where " + campo + " = " + valor.ToString();
                ...
                foreach (DataRow dr in dt.AsEnumerable())
                {
                    ...
                    Indexacion entidad = new Indexacion();
                    foreach (PropertyInfo prop in typeof(Indexacion).GetProperties())
                    {
                        object value = dr[prop.Name];
                        if (value == DBNull.Value) value = null;
