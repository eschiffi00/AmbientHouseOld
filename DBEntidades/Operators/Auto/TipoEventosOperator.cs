...
namespace DbEntidades.Operators
{
    public partial class TipoEventosOperator
    {
        ...
        public static TipoEventos GetOneByParameter(string campo, string valor)
        {
            if (!DbEntidades.Seguridad.Permiso("PermisoTipoEventosBrowse")) throw new PermisoException();
            string columnas = string.Empty;
        ...
        columnas = columnas.Substring(0, columnas.Length - 2);
            DB db = new DB();
            DataTable dt = db.GetDataSet("select " + columnas + " from TipoEventos where  " + campo + " = \'" + valor + "\'").Tables[0];
            TipoEventos TipoEventos = new TipoEventos();
            if (dt.Rows.Count > 0)
            {
                foreach (PropertyInfo prop in typeof(TipoEventos).GetProperties())
        ...
        public static List<TipoEventos> GetAllByParameter(string campo, string valor)
            {
                if (!DbEntidades.Seguridad.Permiso("PermisoTipoEventosBrowse")) throw new PermisoException();
                string columnas = string.Empty;
                ...
                if (tipo == "String")
                {
                    queryStr = "select " + columnas + " from TipoEventos where " + campo + " = \'" + valor.ToString() + "\'";
                }
                else
                {
                    queryStr = "select " + columnas + " from TipoEventos where " + campo + " = " + valor.ToString();
                ...
                foreach (DataRow dr in dt.AsEnumerable())
                {
                    ...
                    TipoEventos entidad = new TipoEventos();
                    foreach (PropertyInfo prop in typeof(TipoEventos).GetProperties())
                    {
                        object value = dr[prop.Name];
                        if (value == DBNull.Value) value = null;
