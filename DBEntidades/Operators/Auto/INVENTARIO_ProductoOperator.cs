...
namespace DbEntidades.Operators
{
    public partial class INVENTARIO_ProductoOperator
    {
        ...
        public static INVENTARIO_Producto GetOneByParameter(string campo, string valor)
        {
            if (!DbEntidades.Seguridad.Permiso("PermisoINVENTARIO_ProductoBrowse")) throw new PermisoException();
            string columnas = string.Empty;
        ...
        columnas = columnas.Substring(0, columnas.Length - 2);
            DB db = new DB();
            DataTable dt = db.GetDataSet("select " + columnas + " from INVENTARIO_Producto where  " + campo + " = \'" + valor + "\'").Tables[0];
            INVENTARIO_Producto INVENTARIO_Producto = new INVENTARIO_Producto();
            if (dt.Rows.Count > 0)
            {
                foreach (PropertyInfo prop in typeof(INVENTARIO_Producto).GetProperties())
        ...
        public static List<INVENTARIO_Producto> GetAllByParameter(string campo, string valor)
            {
                if (!DbEntidades.Seguridad.Permiso("PermisoINVENTARIO_ProductoBrowse")) throw new PermisoException();
                string columnas = string.Empty;
                ...
                if (tipo == "String")
                {
                    queryStr = "select " + columnas + " from INVENTARIO_Producto where " + campo + " = \'" + valor.ToString() + "\'";
                }
                else
                {
                    queryStr = "select " + columnas + " from INVENTARIO_Producto where " + campo + " = " + valor.ToString();
                ...
                foreach (DataRow dr in dt.AsEnumerable())
                {
                    ...
                    INVENTARIO_Producto entidad = new INVENTARIO_Producto();
                    foreach (PropertyInfo prop in typeof(INVENTARIO_Producto).GetProperties())
                    {
                        object value = dr[prop.Name];
                        if (value == DBNull.Value) value = null;
