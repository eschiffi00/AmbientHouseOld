...
namespace DbEntidades.Operators
{
    public partial class PermisoOperator
    {
        ...
        public static Permiso GetOneByParameter(string campo, string valor)
        {
            if (!DbEntidades.Seguridad.Permiso("PermisoPermisoBrowse")) throw new PermisoException();
            string columnas = string.Empty;
        ...
        columnas = columnas.Substring(0, columnas.Length - 2);
            DB db = new DB();
            DataTable dt = db.GetDataSet("select " + columnas + " from Permiso where  " + campo + " = \'" + valor + "\'").Tables[0];
            Permiso Permiso = new Permiso();
            if (dt.Rows.Count > 0)
            {
                foreach (PropertyInfo prop in typeof(Permiso).GetProperties())
        ...
        public static List<Permiso> GetAllByParameter(string campo, string valor)
            {
                if (!DbEntidades.Seguridad.Permiso("PermisoPermisoBrowse")) throw new PermisoException();
                string columnas = string.Empty;
                ...
                if (tipo == "String")
                {
                    queryStr = "select " + columnas + " from Permiso where " + campo + " = \'" + valor.ToString() + "\'";
                }
                else
                {
                    queryStr = "select " + columnas + " from Permiso where " + campo + " = " + valor.ToString();
