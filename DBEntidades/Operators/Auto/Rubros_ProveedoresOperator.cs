...
namespace DbEntidades.Operators
{
    public partial class Rubros_ProveedoresOperator
    {
        ...
        public static Rubros_Proveedores GetOneByParameter(string campo, string valor)
        {
            if (!DbEntidades.Seguridad.Permiso("PermisoRubros_ProveedoresBrowse")) throw new PermisoException();
            string columnas = string.Empty;
        ...
        columnas = columnas.Substring(0, columnas.Length - 2);
            DB db = new DB();
            DataTable dt = db.GetDataSet("select " + columnas + " from Rubros_Proveedores where  " + campo + " = \'" + valor + "\'").Tables[0];
            Rubros_Proveedores Rubros_Proveedores = new Rubros_Proveedores();
            if (dt.Rows.Count > 0)
            {
                foreach (PropertyInfo prop in typeof(Rubros_Proveedores).GetProperties())
        ...
        public static List<Rubros_Proveedores> GetAllByParameter(string campo, string valor)
            {
                if (!DbEntidades.Seguridad.Permiso("PermisoRubros_ProveedoresBrowse")) throw new PermisoException();
                string columnas = string.Empty;
                ...
                if (tipo == "String")
                {
                    queryStr = "select " + columnas + " from Rubros_Proveedores where " + campo + " = \'" + valor.ToString() + "\'";
                }
                else
                {
                    queryStr = "select " + columnas + " from Rubros_Proveedores where " + campo + " = " + valor.ToString();
                ...
                foreach (DataRow dr in dt.AsEnumerable())
                {
                    ...
                    Rubros_Proveedores entidad = new Rubros_Proveedores();
                    foreach (PropertyInfo prop in typeof(Rubros_Proveedores).GetProperties())
                    {
                        object value = dr[prop.Name];
                        if (value == DBNull.Value) value = null;
