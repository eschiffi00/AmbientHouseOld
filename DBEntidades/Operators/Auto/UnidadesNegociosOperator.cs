...
namespace DbEntidades.Operators
{
    public partial class UnidadesNegociosOperator
    {
        ...
        public static UnidadesNegocios GetOneByParameter(string campo, string valor)
        {
            if (!DbEntidades.Seguridad.Permiso("PermisoUnidadesNegociosBrowse")) throw new PermisoException();
            string columnas = string.Empty;
        ...
        columnas = columnas.Substring(0, columnas.Length - 2);
            DB db = new DB();
            DataTable dt = db.GetDataSet("select " + columnas + " from UnidadesNegocios where  " + campo + " = \'" + valor + "\'").Tables[0];
            UnidadesNegocios UnidadesNegocios = new UnidadesNegocios();
            if (dt.Rows.Count > 0)
            {
                foreach (PropertyInfo prop in typeof(UnidadesNegocios).GetProperties())
        ...
        public static List<UnidadesNegocios> GetAllByParameter(string campo, string valor)
            {
                if (!DbEntidades.Seguridad.Permiso("PermisoUnidadesNegociosBrowse")) throw new PermisoException();
                string columnas = string.Empty;
                ...
                if (tipo == "String")
                {
                    queryStr = "select " + columnas + " from UnidadesNegocios where " + campo + " = \'" + valor.ToString() + "\'";
                }
                else
                {
                    queryStr = "select " + columnas + " from UnidadesNegocios where " + campo + " = " + valor.ToString();
                ...
                foreach (DataRow dr in dt.AsEnumerable())
                {
                    ...
                    UnidadesNegocios entidad = new UnidadesNegocios();
                    foreach (PropertyInfo prop in typeof(UnidadesNegocios).GetProperties())
                    {
                        object value = dr[prop.Name];
                        if (value == DBNull.Value) value = null;
