...
namespace DbEntidades.Operators
{
    public partial class UsuarioRolOperator
    {
        ...
        public static UsuarioRol GetOneByParameter(string campo, string valor)
        {
            if (!DbEntidades.Seguridad.Permiso("PermisoUsuarioRolBrowse")) throw new PermisoException();
            string columnas = string.Empty;
        ...
        columnas = columnas.Substring(0, columnas.Length - 2);
            DB db = new DB();
            DataTable dt = db.GetDataSet("select " + columnas + " from UsuarioRol where  " + campo + " = \'" + valor + "\'").Tables[0];
            UsuarioRol UsuarioRol = new UsuarioRol();
            if (dt.Rows.Count > 0)
            {
                foreach (PropertyInfo prop in typeof(UsuarioRol).GetProperties())
        ...
        public static List<UsuarioRol> GetAllByParameter(string campo, string valor)
            {
                if (!DbEntidades.Seguridad.Permiso("PermisoUsuarioRolBrowse")) throw new PermisoException();
                string columnas = string.Empty;
                ...
                if (tipo == "String")
                {
                    queryStr = "select " + columnas + " from UsuarioRol where " + campo + " = \'" + valor.ToString() + "\'";
                }
                else
                {
                    queryStr = "select " + columnas + " from UsuarioRol where " + campo + " = " + valor.ToString();
