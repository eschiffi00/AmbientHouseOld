...
namespace DbEntidades.Operators
{
    public partial class PagosClientesOperator
    {
        ...
        public static PagosClientes GetOneByParameter(string campo, string valor)
        {
            if (!DbEntidades.Seguridad.Permiso("PermisoPagosClientesBrowse")) throw new PermisoException();
            string columnas = string.Empty;
        ...
        columnas = columnas.Substring(0, columnas.Length - 2);
            DB db = new DB();
            DataTable dt = db.GetDataSet("select " + columnas + " from PagosClientes where  " + campo + " = \'" + valor + "\'").Tables[0];
            PagosClientes PagosClientes = new PagosClientes();
            if (dt.Rows.Count > 0)
            {
                foreach (PropertyInfo prop in typeof(PagosClientes).GetProperties())
        ...
        public static List<PagosClientes> GetAllByParameter(string campo, string valor)
            {
                if (!DbEntidades.Seguridad.Permiso("PermisoPagosClientesBrowse")) throw new PermisoException();
                string columnas = string.Empty;
                ...
                if (tipo == "String")
                {
                    queryStr = "select " + columnas + " from PagosClientes where " + campo + " = \'" + valor.ToString() + "\'";
                }
                else
                {
                    queryStr = "select " + columnas + " from PagosClientes where " + campo + " = " + valor.ToString();
                ...
                foreach (DataRow dr in dt.AsEnumerable())
                {
                    ...
                    PagosClientes entidad = new PagosClientes();
                    foreach (PropertyInfo prop in typeof(PagosClientes).GetProperties())
                    {
                        object value = dr[prop.Name];
                        if (value == DBNull.Value) value = null;
