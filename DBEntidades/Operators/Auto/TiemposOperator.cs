...
namespace DbEntidades.Operators
{
    public partial class TiemposOperator
    {
        ...
        public static Tiempos GetOneByParameter(string campo, string valor)
        {
            if (!DbEntidades.Seguridad.Permiso("PermisoTiemposBrowse")) throw new PermisoException();
            string columnas = string.Empty;
        ...
        columnas = columnas.Substring(0, columnas.Length - 2);
            DB db = new DB();
            DataTable dt = db.GetDataSet("select " + columnas + " from Tiempos where  " + campo + " = \'" + valor + "\'").Tables[0];
            Tiempos Tiempos = new Tiempos();
            if (dt.Rows.Count > 0)
            {
                foreach (PropertyInfo prop in typeof(Tiempos).GetProperties())
        ...
        public static List<Tiempos> GetAllByParameter(string campo, string valor)
            {
                if (!DbEntidades.Seguridad.Permiso("PermisoTiemposBrowse")) throw new PermisoException();
                string columnas = string.Empty;
                ...
                if (tipo == "String")
                {
                    queryStr = "select " + columnas + " from Tiempos where " + campo + " = \'" + valor.ToString() + "\'";
                }
                else
                {
                    queryStr = "select " + columnas + " from Tiempos where " + campo + " = " + valor.ToString();
