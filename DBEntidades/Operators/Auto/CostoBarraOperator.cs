...
namespace DbEntidades.Operators
{
    public partial class CostoBarraOperator
    {
        ...
        public static CostoBarra GetOneByParameter(string campo, string valor)
        {
            if (!DbEntidades.Seguridad.Permiso("PermisoCostoBarraBrowse")) throw new PermisoException();
            string columnas = string.Empty;
        ...
        columnas = columnas.Substring(0, columnas.Length - 2);
            DB db = new DB();
            DataTable dt = db.GetDataSet("select " + columnas + " from CostoBarra where  " + campo + " = \'" + valor + "\'").Tables[0];
            CostoBarra CostoBarra = new CostoBarra();
            if (dt.Rows.Count > 0)
            {
                foreach (PropertyInfo prop in typeof(CostoBarra).GetProperties())
        ...
        public static List<CostoBarra> GetAllByParameter(string campo, string valor)
            {
                if (!DbEntidades.Seguridad.Permiso("PermisoCostoBarraBrowse")) throw new PermisoException();
                string columnas = string.Empty;
                ...
                if (tipo == "String")
                {
                    queryStr = "select " + columnas + " from CostoBarra where " + campo + " = \'" + valor.ToString() + "\'";
                }
                else
                {
                    queryStr = "select " + columnas + " from CostoBarra where " + campo + " = " + valor.ToString();
                ...
                foreach (DataRow dr in dt.AsEnumerable())
                {
                    ...
                    CostoBarra entidad = new CostoBarra();
                    foreach (PropertyInfo prop in typeof(CostoBarra).GetProperties())
                    {
                        object value = dr[prop.Name];
                        if (value == DBNull.Value) value = null;
