...
namespace DbEntidades.Operators
{
    public partial class DuracionEventoOperator
    {
        ...
        public static DuracionEvento GetOneByParameter(string campo, string valor)
        {
            if (!DbEntidades.Seguridad.Permiso("PermisoDuracionEventoBrowse")) throw new PermisoException();
            string columnas = string.Empty;
        ...
        columnas = columnas.Substring(0, columnas.Length - 2);
            DB db = new DB();
            DataTable dt = db.GetDataSet("select " + columnas + " from DuracionEvento where  " + campo + " = \'" + valor + "\'").Tables[0];
            DuracionEvento DuracionEvento = new DuracionEvento();
            if (dt.Rows.Count > 0)
            {
                foreach (PropertyInfo prop in typeof(DuracionEvento).GetProperties())
        ...
        public static List<DuracionEvento> GetAllByParameter(string campo, string valor)
            {
                if (!DbEntidades.Seguridad.Permiso("PermisoDuracionEventoBrowse")) throw new PermisoException();
                string columnas = string.Empty;
                ...
                if (tipo == "String")
                {
                    queryStr = "select " + columnas + " from DuracionEvento where " + campo + " = \'" + valor.ToString() + "\'";
                }
                else
                {
                    queryStr = "select " + columnas + " from DuracionEvento where " + campo + " = " + valor.ToString();
                ...
                foreach (DataRow dr in dt.AsEnumerable())
                {
                    ...
                    DuracionEvento entidad = new DuracionEvento();
                    foreach (PropertyInfo prop in typeof(DuracionEvento).GetProperties())
                    {
                        object value = dr[prop.Name];
                        if (value == DBNull.Value) value = null;
