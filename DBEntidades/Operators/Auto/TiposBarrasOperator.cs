...
namespace DbEntidades.Operators
{
    public partial class TiposBarrasOperator
    {
        ...
        public static List<TiposBarras> GetAllByParameter(string campo, string valor)
            {
                if (!DbEntidades.Seguridad.Permiso("PermisoTiposBarrasBrowse")) throw new PermisoException();
                string columnas = string.Empty;
                ...
                if (tipo == "String")
                {
                    queryStr = "select " + columnas + " from TiposBarras where " + campo + " = \'" + valor.ToString() + "\'";
                }
                else
                {
                    queryStr = "select " + columnas + " from TiposBarras where " + campo + " = " + valor.ToString();
                ...
                foreach (DataRow dr in dt.AsEnumerable())
                {
                    ...
                    TiposBarras entidad = new TiposBarras();
                    foreach (PropertyInfo prop in typeof(TiposBarras).GetProperties())
                    {
                        object value = dr[prop.Name];
                        if (value == DBNull.Value) value = null;
