...
namespace DbEntidades.Operators
{
    public partial class CostosPaquetesCIAmbientacionOperator
    {
        ...
        public static CostosPaquetesCIAmbientacion GetOneByParameter(string campo, string valor)
        {
            if (!DbEntidades.Seguridad.Permiso("PermisoCostosPaquetesCIAmbientacionBrowse")) throw new PermisoException();
            string columnas = string.Empty;
        ...
        columnas = columnas.Substring(0, columnas.Length - 2);
            DB db = new DB();
            DataTable dt = db.GetDataSet("select " + columnas + " from CostosPaquetesCIAmbientacion where  " + campo + " = \'" + valor + "\'").Tables[0];
            CostosPaquetesCIAmbientacion CostosPaquetesCIAmbientacion = new CostosPaquetesCIAmbientacion();
            if (dt.Rows.Count > 0)
            {
                foreach (PropertyInfo prop in typeof(CostosPaquetesCIAmbientacion).GetProperties())
        ...
        public static List<CostosPaquetesCIAmbientacion> GetAllByParameter(string campo, string valor)
            {
                if (!DbEntidades.Seguridad.Permiso("PermisoCostosPaquetesCIAmbientacionBrowse")) throw new PermisoException();
                string columnas = string.Empty;
                ...
                if (tipo == "String")
                {
                    queryStr = "select " + columnas + " from CostosPaquetesCIAmbientacion where " + campo + " = \'" + valor.ToString() + "\'";
                }
                else
                {
                    queryStr = "select " + columnas + " from CostosPaquetesCIAmbientacion where " + campo + " = " + valor.ToString();
                ...
                foreach (DataRow dr in dt.AsEnumerable())
                {
                    ...
                    CostosPaquetesCIAmbientacion entidad = new CostosPaquetesCIAmbientacion();
                    foreach (PropertyInfo prop in typeof(CostosPaquetesCIAmbientacion).GetProperties())
                    {
                        object value = dr[prop.Name];
                        if (value == DBNull.Value) value = null;
