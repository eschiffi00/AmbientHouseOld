...
namespace DbEntidades.Operators
{
    public partial class CiudadesOperator
    {
        ...
        public static Ciudades GetOneByIdentity(int Id)
        {
            ...
            DB db = new DB();
            DataTable dt = db.GetDataSet("select " + columnas + " from Ciudades where  = " + Id.ToString()).Tables[0];
        ...
        public static Ciudades GetOneByParameter(string campo, string valor)
        {
            if (!DbEntidades.Seguridad.Permiso("PermisoCiudadesBrowse")) throw new PermisoException();
            string columnas = string.Empty;
        ...
        columnas = columnas.Substring(0, columnas.Length - 2);
            DB db = new DB();
            DataTable dt = db.GetDataSet("select " + columnas + " from Ciudades where  " + campo + " = \'" + valor + "\'").Tables[0];
            Ciudades Ciudades = new Ciudades();
            if (dt.Rows.Count > 0)
            {
                foreach (PropertyInfo prop in typeof(Ciudades).GetProperties())
        ...
        public static List<Ciudades> GetAllByParameter(string campo, string valor)
            {
                if (!DbEntidades.Seguridad.Permiso("PermisoCiudadesBrowse")) throw new PermisoException();
                string columnas = string.Empty;
                ...
                if (tipo == "String")
                {
                    queryStr = "select " + columnas + " from Ciudades where " + campo + " = \'" + valor.ToString() + "\'";
                }
                else
                {
                    queryStr = "select " + columnas + " from Ciudades where " + campo + " = " + valor.ToString();
