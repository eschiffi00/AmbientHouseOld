...
namespace DbEntidades.Operators
{
    public partial class PresupuestosOperator
    {
        ...
        public static Presupuestos GetOneByParameter(string campo, string valor)
        {
            if (!DbEntidades.Seguridad.Permiso("PermisoPresupuestosBrowse")) throw new PermisoException();
            string columnas = string.Empty;
        ...
        columnas = columnas.Substring(0, columnas.Length - 2);
            DB db = new DB();
            DataTable dt = db.GetDataSet("select " + columnas + " from Presupuestos where  " + campo + " = \'" + valor + "\'").Tables[0];
            Presupuestos Presupuestos = new Presupuestos();
            if (dt.Rows.Count > 0)
            {
                foreach (PropertyInfo prop in typeof(Presupuestos).GetProperties())
        ...
        public static List<Presupuestos> GetAllByParameter(string campo, string valor)
            {
                if (!DbEntidades.Seguridad.Permiso("PermisoPresupuestosBrowse")) throw new PermisoException();
                string columnas = string.Empty;
                ...
                if (tipo == "String")
                {
                    queryStr = "select " + columnas + " from Presupuestos where " + campo + " = \'" + valor.ToString() + "\'";
                }
                else
                {
                    queryStr = "select " + columnas + " from Presupuestos where " + campo + " = " + valor.ToString();
                ...
                foreach (DataRow dr in dt.AsEnumerable())
                {
                    ...
                    Presupuestos entidad = new Presupuestos();
                    foreach (PropertyInfo prop in typeof(Presupuestos).GetProperties())
                    {
                        object value = dr[prop.Name];
                        if (value == DBNull.Value) value = null;
