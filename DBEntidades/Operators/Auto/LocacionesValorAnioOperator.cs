...
namespace DbEntidades.Operators
{
    public partial class LocacionesValorAnioOperator
    {
        ...
        public static LocacionesValorAnio GetOneByParameter(string campo, string valor)
        {
            if (!DbEntidades.Seguridad.Permiso("PermisoLocacionesValorAnioBrowse")) throw new PermisoException();
            string columnas = string.Empty;
        ...
        columnas = columnas.Substring(0, columnas.Length - 2);
            DB db = new DB();
            DataTable dt = db.GetDataSet("select " + columnas + " from LocacionesValorAnio where  " + campo + " = \'" + valor + "\'").Tables[0];
            LocacionesValorAnio LocacionesValorAnio = new LocacionesValorAnio();
            if (dt.Rows.Count > 0)
            {
                foreach (PropertyInfo prop in typeof(LocacionesValorAnio).GetProperties())
        ...
        public static List<LocacionesValorAnio> GetAllByParameter(string campo, string valor)
            {
                if (!DbEntidades.Seguridad.Permiso("PermisoLocacionesValorAnioBrowse")) throw new PermisoException();
                string columnas = string.Empty;
                ...
                if (tipo == "String")
                {
                    queryStr = "select " + columnas + " from LocacionesValorAnio where " + campo + " = \'" + valor.ToString() + "\'";
                }
                else
                {
                    queryStr = "select " + columnas + " from LocacionesValorAnio where " + campo + " = " + valor.ToString();
                ...
                foreach (DataRow dr in dt.AsEnumerable())
                {
                    ...
                    LocacionesValorAnio entidad = new LocacionesValorAnio();
                    foreach (PropertyInfo prop in typeof(LocacionesValorAnio).GetProperties())
                    {
                        object value = dr[prop.Name];
                        if (value == DBNull.Value) value = null;
