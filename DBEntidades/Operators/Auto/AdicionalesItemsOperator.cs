...
namespace DbEntidades.Operators
{
    public partial class AdicionalesItemsOperator
    {
        ...
        public static AdicionalesItems GetOneByParameter(string campo, string valor)
        {
            if (!DbEntidades.Seguridad.Permiso("PermisoAdicionalesItemsBrowse")) throw new PermisoException();
            string columnas = string.Empty;
        ...
        columnas = columnas.Substring(0, columnas.Length - 2);
            DB db = new DB();
            DataTable dt = db.GetDataSet("select " + columnas + " from AdicionalesItems where  " + campo + " = \'" + valor + "\'").Tables[0];
            AdicionalesItems AdicionalesItems = new AdicionalesItems();
            if (dt.Rows.Count > 0)
            {
                foreach (PropertyInfo prop in typeof(AdicionalesItems).GetProperties())
        ...
        public static List<AdicionalesItems> GetAllByParameter(string campo, string valor)
            {
                if (!DbEntidades.Seguridad.Permiso("PermisoAdicionalesItemsBrowse")) throw new PermisoException();
                string columnas = string.Empty;
                ...
                if (tipo == "String")
                {
                    queryStr = "select " + columnas + " from AdicionalesItems where " + campo + " = \'" + valor.ToString() + "\'";
                }
                else
                {
                    queryStr = "select " + columnas + " from AdicionalesItems where " + campo + " = " + valor.ToString();
                ...
                foreach (DataRow dr in dt.AsEnumerable())
                {
                    ...
                    AdicionalesItems entidad = new AdicionalesItems();
                    foreach (PropertyInfo prop in typeof(AdicionalesItems).GetProperties())
                    {
                        object value = dr[prop.Name];
                        if (value == DBNull.Value) value = null;
