...
namespace DbEntidades.Operators
{
    public partial class INVENTARIO_Requerimiento_DetalleOperator
    {
        ...
        public static INVENTARIO_Requerimiento_Detalle GetOneByParameter(string campo, string valor)
        {
            if (!DbEntidades.Seguridad.Permiso("PermisoINVENTARIO_Requerimiento_DetalleBrowse")) throw new PermisoException();
            string columnas = string.Empty;
        ...
        columnas = columnas.Substring(0, columnas.Length - 2);
            DB db = new DB();
            DataTable dt = db.GetDataSet("select " + columnas + " from INVENTARIO_Requerimiento_Detalle where  " + campo + " = \'" + valor + "\'").Tables[0];
            INVENTARIO_Requerimiento_Detalle INVENTARIO_Requerimiento_Detalle = new INVENTARIO_Requerimiento_Detalle();
            if (dt.Rows.Count > 0)
            {
                foreach (PropertyInfo prop in typeof(INVENTARIO_Requerimiento_Detalle).GetProperties())
        ...
        public static List<INVENTARIO_Requerimiento_Detalle> GetAllByParameter(string campo, string valor)
            {
                if (!DbEntidades.Seguridad.Permiso("PermisoINVENTARIO_Requerimiento_DetalleBrowse")) throw new PermisoException();
                string columnas = string.Empty;
                ...
                if (tipo == "String")
                {
                    queryStr = "select " + columnas + " from INVENTARIO_Requerimiento_Detalle where " + campo + " = \'" + valor.ToString() + "\'";
                }
                else
                {
                    queryStr = "select " + columnas + " from INVENTARIO_Requerimiento_Detalle where " + campo + " = " + valor.ToString();
                ...
                foreach (DataRow dr in dt.AsEnumerable())
                {
                    ...
                    INVENTARIO_Requerimiento_Detalle entidad = new INVENTARIO_Requerimiento_Detalle();
                    foreach (PropertyInfo prop in typeof(INVENTARIO_Requerimiento_Detalle).GetProperties())
                    {
                        object value = dr[prop.Name];
                        if (value == DBNull.Value) value = null;
