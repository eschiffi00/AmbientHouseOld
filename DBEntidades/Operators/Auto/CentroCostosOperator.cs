...
namespace DbEntidades.Operators
{
    public partial class CentroCostosOperator
    {
        ...
        public static CentroCostos GetOneByParameter(string campo, string valor)
        {
            if (!DbEntidades.Seguridad.Permiso("PermisoCentroCostosBrowse")) throw new PermisoException();
            string columnas = string.Empty;
        ...
        columnas = columnas.Substring(0, columnas.Length - 2);
            DB db = new DB();
            DataTable dt = db.GetDataSet("select " + columnas + " from CentroCostos where  " + campo + " = \'" + valor + "\'").Tables[0];
            CentroCostos CentroCostos = new CentroCostos();
            if (dt.Rows.Count > 0)
            {
                foreach (PropertyInfo prop in typeof(CentroCostos).GetProperties())
        ...
        public static List<CentroCostos> GetAllByParameter(string campo, string valor)
            {
                if (!DbEntidades.Seguridad.Permiso("PermisoCentroCostosBrowse")) throw new PermisoException();
                string columnas = string.Empty;
                ...
                if (tipo == "String")
                {
                    queryStr = "select " + columnas + " from CentroCostos where " + campo + " = \'" + valor.ToString() + "\'";
                }
                else
                {
                    queryStr = "select " + columnas + " from CentroCostos where " + campo + " = " + valor.ToString();
                ...
                foreach (DataRow dr in dt.AsEnumerable())
                {
                    ...
                    CentroCostos entidad = new CentroCostos();
                    foreach (PropertyInfo prop in typeof(CentroCostos).GetProperties())
                    {
                        object value = dr[prop.Name];
                        if (value == DBNull.Value) value = null;
