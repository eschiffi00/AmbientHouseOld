...
namespace DbEntidades.Operators
{
    public partial class SectoresOperator
    {
        ...
        public static Sectores GetOneByParameter(string campo, string valor)
        {
            if (!DbEntidades.Seguridad.Permiso("PermisoSectoresBrowse")) throw new PermisoException();
            string columnas = string.Empty;
        ...
        columnas = columnas.Substring(0, columnas.Length - 2);
            DB db = new DB();
            DataTable dt = db.GetDataSet("select " + columnas + " from Sectores where  " + campo + " = \'" + valor + "\'").Tables[0];
            Sectores Sectores = new Sectores();
            if (dt.Rows.Count > 0)
            {
                foreach (PropertyInfo prop in typeof(Sectores).GetProperties())
        ...
        public static List<Sectores> GetAllByParameter(string campo, string valor)
            {
                if (!DbEntidades.Seguridad.Permiso("PermisoSectoresBrowse")) throw new PermisoException();
                string columnas = string.Empty;
                ...
                if (tipo == "String")
                {
                    queryStr = "select " + columnas + " from Sectores where " + campo + " = \'" + valor.ToString() + "\'";
                }
                else
                {
                    queryStr = "select " + columnas + " from Sectores where " + campo + " = " + valor.ToString();
