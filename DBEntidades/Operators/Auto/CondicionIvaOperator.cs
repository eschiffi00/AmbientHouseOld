...
namespace DbEntidades.Operators
{
    public partial class CondicionIvaOperator
    {
        ...
        public static CondicionIva GetOneByParameter(string campo, string valor)
        {
            if (!DbEntidades.Seguridad.Permiso("PermisoCondicionIvaBrowse")) throw new PermisoException();
            string columnas = string.Empty;
        ...
        columnas = columnas.Substring(0, columnas.Length - 2);
            DB db = new DB();
            DataTable dt = db.GetDataSet("select " + columnas + " from CondicionIva where  " + campo + " = \'" + valor + "\'").Tables[0];
            CondicionIva CondicionIva = new CondicionIva();
            if (dt.Rows.Count > 0)
            {
                foreach (PropertyInfo prop in typeof(CondicionIva).GetProperties())
        ...
        public static List<CondicionIva> GetAllByParameter(string campo, string valor)
            {
                if (!DbEntidades.Seguridad.Permiso("PermisoCondicionIvaBrowse")) throw new PermisoException();
                string columnas = string.Empty;
                ...
                if (tipo == "String")
                {
                    queryStr = "select " + columnas + " from CondicionIva where " + campo + " = \'" + valor.ToString() + "\'";
                }
                else
                {
                    queryStr = "select " + columnas + " from CondicionIva where " + campo + " = " + valor.ToString();
                ...
                foreach (DataRow dr in dt.AsEnumerable())
                {
                    ...
                    CondicionIva entidad = new CondicionIva();
                    foreach (PropertyInfo prop in typeof(CondicionIva).GetProperties())
                    {
                        object value = dr[prop.Name];
                        if (value == DBNull.Value) value = null;
