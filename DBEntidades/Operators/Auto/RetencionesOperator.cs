...
namespace DbEntidades.Operators
{
    public partial class RetencionesOperator
    {
        ...
        public static Retenciones GetOneByParameter(string campo, string valor)
        {
            if (!DbEntidades.Seguridad.Permiso("PermisoRetencionesBrowse")) throw new PermisoException();
            string columnas = string.Empty;
        ...
        columnas = columnas.Substring(0, columnas.Length - 2);
            DB db = new DB();
            DataTable dt = db.GetDataSet("select " + columnas + " from Retenciones where  " + campo + " = \'" + valor + "\'").Tables[0];
            Retenciones Retenciones = new Retenciones();
            if (dt.Rows.Count > 0)
            {
                foreach (PropertyInfo prop in typeof(Retenciones).GetProperties())
        ...
        public static List<Retenciones> GetAllByParameter(string campo, string valor)
            {
                if (!DbEntidades.Seguridad.Permiso("PermisoRetencionesBrowse")) throw new PermisoException();
                string columnas = string.Empty;
                ...
                if (tipo == "String")
                {
                    queryStr = "select " + columnas + " from Retenciones where " + campo + " = \'" + valor.ToString() + "\'";
                }
                else
                {
                    queryStr = "select " + columnas + " from Retenciones where " + campo + " = " + valor.ToString();
                ...
                foreach (DataRow dr in dt.AsEnumerable())
                {
                    ...
                    Retenciones entidad = new Retenciones();
                    foreach (PropertyInfo prop in typeof(Retenciones).GetProperties())
                    {
                        object value = dr[prop.Name];
                        if (value == DBNull.Value) value = null;
