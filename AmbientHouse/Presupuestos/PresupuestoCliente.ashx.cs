...
{
    /// <summary>
    /// Summary description for PresupuestoCliente
    /// </summary>
    public class PresupuestoCliente : IHttpHandler, IRequiresSessionState
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            try
            {
                ...
                if (Presupuestos != null)
                {
                    ...
                    List<PresupuestoDetalle> presupuestoSalon = servicio.ObtenerPresupuestoDetalle((int)Presupuestos.PresupuestoId, RubroSalon, pendiente);
                    ...
                    foreach (var item in presupuestoSalon)
                    {
                        ...
                        if (item.PrecioSeleccionado == "Sin Precio")
                        {
                            ...
                            pdfFormFields.SetField("SalonB", Convert.ToString("No Incluido"));
                        }
                        else
                        { pdfFormFields.SetField("Salon", Convert.ToString(locacion.Descripcion)); }
                    ...
                    List<PresupuestoDetalle> presupuestoTecnica = servicio.ObtenerPresupuestoDetalle((int)Presupuestos.PresupuestoId, RubroTecnica, pendiente);
                    ...
                    List<PresupuestoDetalle> presupuestoCatering = servicio.ObtenerPresupuestoDetalle((int)Presupuestos.PresupuestoId, RubroCatering, pendiente);
                    ...
                    List<PresupuestoDetalle> presupuestoBarra = servicio.ObtenerPresupuestoDetalle((int)Presupuestos.PresupuestoId, RubroBarra, pendiente);
                    ...
                    List<PresupuestoDetalle> presupuestoAmbientacion = servicio.ObtenerPresupuestoDetalle((int)Presupuestos.PresupuestoId, RubroAmbientacion, pendiente);
                    ...
                    if (TotalDescuento > 0)
                    {
                        ...
                        pdfFormFields.SetField("TotalDescuento", formatoNumero((double)TotalDescuento));
                    }
                    else
                    {
                    ...
                    if (Presupuestos.ValorOrganizador > 0)
                        ValorOrganizador = double.Parse(Presupuestos.ValorOrganizador.ToString());

                    if ((precioConsolidado + ValorOrganizador) < PrecioTotal)
                    {
                        //pdfFormFields.SetField("TotalConsolidado", Convert.ToString(Math.Ceiling(PrecioTotal)));
                        pdfFormFields.SetField("TotalConsolidado", formatoNumero(Math.Ceiling(PrecioTotal)));
                    }
                    else
                    {
                        //pdfFormFields.SetField("TotalConsolidado", Convert.ToString(Math.Ceiling(precioConsolidado + ValorOrganizador)));
                        pdfFormFields.SetField("TotalConsolidado", formatoNumero(Math.Ceiling(precioConsolidado + ValorOrganizador)));
                    }

                    //BarcodeQRCode QR = new BarcodeQRCode("http", 1000, 1000, null);

                    //Image codeQR = QR.GetImage();


                    ////byte[] imageArray = System.IO.File.ReadAllBytes("la url de la imagen");
                    ////string img = Convert.ToBase64String(imageArray);
                    ////pdfFormFields.SetField("QR", img);

                    //pdfFormFields.GetFieldPositions("QR");

                    //pdfFormFields.SetField("QR", codeQR);
